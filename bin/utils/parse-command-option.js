 ...
 * @param {boolean | number | string} [defaultValue=false] - 可选，默认值. Default is
 *   `false`
 * @param {string} [name=''] - 可选，参数名称. Default is `''`
 * @returns {boolean | string | number} - 解析后的配置参数值
 */
const parseCommandOption = (value, defaultValue = false, name = '') => {
  // 解析 startIndex 的数值
  if (name === '--start-index') {
    return parseNumberOption(value, defaultValue, name);
  }
  ...
  if (name === '--index-length') {
    return value === 'auto'
      ? 'auto'
      : parseNumberOption(value, defaultValue, name);
  }
