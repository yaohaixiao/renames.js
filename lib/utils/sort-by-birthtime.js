...
/**
 * # 将文件数组中的数据按创建时间排序，返回排序后的数组
 *
 * @function sortByBirthtime
...
const sortByBirthtime = (files, order = DEFAULT_ORDER, dirPath = '') =>
  files.toSorted((prev, next) => {
    const prevStats = lstatSync(resolve(dirPath, prev));
    const nextStats = lstatSync(resolve(dirPath, next));
...
export default sortByBirthtime;
