...
import getFilenameByNamesList from './get-filename-by-names-list.js';
 ...
 *   `true`
 * @param {number} [options.indexLength='auto'] - 可选，自动补齐的字符串长度值. Default is
 *   `'auto'`
...
const generateFilename = (oldFilename, index, options = {}) => {
  ...
  if (namesList.length > 0) {
    newFilename = getFilenameByNamesList(
      namesList,
      index,
      normalizedOptions,
      oldFilename,
