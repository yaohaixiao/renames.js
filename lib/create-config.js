...
import getConfigInteractive from '../bin/utils/get-config-interactive.js';
 ...
 * @param {string} [options.dirPath=''] - 可选，目标文件夹路径（绝对路径或相对路径）. Default is `''`
 * @param {string} [options.filesList=''] - 可选，目标文件列表数组数据. Default is `''`
 * @param {string} [options.namesList=''] -
 *   可选，要修改的对应的文件名列表数组或者文件名列表的数据（例如：标题1，标题2）. Default is `''`
 * @param {boolean} [options.cache=false] 可选，是否开启缓存. Default is `false`
 * @returns {Promise<InitCommandOptions | CancelConfirm>} - 返回配置信息的 JSON 数据
 */
const createConfig = async (options) => {
  ...
  // 优先使用传入的 options 的 filesList 值
  if (options.filesList) {
    config.filesList = options.filesList;
  } else {
    // 然后使用交互式获取 dir 参数
    await getConfigInteractive(
      config,
      'filesList',
      '请输入需要执行重命名操作的文件列表数组数据',
      DEMO_FILES_DATA,
    );
  }
