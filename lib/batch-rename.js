...
const getNormalizedOptions = (options, filesCount) => {
  ...
  // 处理排序函数（确保为有效函数或 null）
  config.sortBy = isFunction(config.sortBy) ? config.sortBy : null;
...
// 辅助函数：处理单个文件的重命名逻辑（抽离 for 循环内的复杂逻辑）
const doRename = (folderPath, oldFilename, index, config) => {
  const { join } = path;
  ...
  // 确定用于生成新文件名的核心参数
  const { names, autoIndex, force = false } = config;
  const namesCount = names.length;
  const basename = getBasename(oldFilename);
  const name = namesCount && !autoIndex ? names[index] || basename : index;

  // 生成新文件名并执行重命名
  const newFilename = generateFilename(oldFilename, name, config);
...
const batchRename = (folderPath, options = {}) => {
  ...
  if (!isFileExists(absolutePath)) {
    showWarningLog('警告', absolutePath, '文件夹不存在或已被删除。');
    return false;
  }

  // 读取文件夹文件并校验是否为空
  const files = readDir(absolutePath);

  if (files.length === 0) {
    showWarningLog('警告', absolutePath, '文件夹中没有任何文件。');
    return false;
  }

  // 规整配置并过滤目标文件
  const normalizedOptions = getNormalizedOptions(options, files.length);
  const { filter, sortBy } = normalizedOptions;
  const results = filter ? filter(files) : files;

  if (results.length === 0) {
    showWarningLog('警告', absolutePath, '文件夹中没有符合过滤条件的文件。');
    return false;
  }
  ...
  for (const [index, oldFilename] of sortedFiles.entries()) {
    doRename(absolutePath, oldFilename, index, normalizedOptions);
  }
