...
import analysisDirFiles from './utils/analysis-dir-files.js';
import analysisNamesList from './utils/analysis-names-list.js';
import cacheRenames from './utils/cache-renames.js';
 ...
 * @param {Array} [options.files=[]] - 可选，要修改的对应的文件名列表数组或者文件名列表的数据. Default is
 *   `[]`
 ...
 */
const batchRename = (dirFiles, options = {}) => {
  const {
    files = [],
    finalFiles = [],
    finalDirPath = '',
  } = analysisDirFiles(dirFiles, options);

  if (finalFiles.length === 0) {
    showWarningLog('警告', '未检测到任何文件需要重命名', '已退出程序');
    return false;
  }

  const { length } = finalFiles.length.toString();
  ...
  const results = filter ? filter(finalFiles) : finalFiles;

  if (results.length === 0) {
    showWarningLog('警告', '没有符合过滤条件的文件', '已退出程序');
    return false;
  }

  // 加载并校验文件名列表
  normalizedOptions.names = analysisNamesList(
    normalizedOptions,
    results.length,
  );
  ...
  // 开启缓存，将缓存数据写入 renames.cache.json 文件
  if (cache) {
    cacheRenames(finalDirPath, files, records);
  }
